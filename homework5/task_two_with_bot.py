...
def ask_turn_with_bot(number_turn, max_num):
    if is_gamer_two(number_turn):
        bot_value = random.randint(1, max_num)
        print(f"Бот взял {bot_value}")
        return  bot_value
...
if __name__ == "__main__":
    ...
    while number_of_sweets > 0:
        value = ask_turn_with_bot(turn_number, MAX_CURRENT)
        while value > MAX_CURRENT:
            value = ask_turn_with_bot(turn_number, MAX_CURRENT)
        while value > number_of_sweets:
            value = ask_turn_with_bot(turn_number, number_of_sweets)
